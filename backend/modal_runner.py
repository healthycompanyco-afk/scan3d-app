...
def trigger_reconstruction(model_id: str, user_id: str, input_type: str):
    ...
    if input_type == "ai_single":
        # IA de 1 foto → TRELLIS (alta qualidade). Fallback para TripoSR se TRELLIS
        # não estiver configurado.
        endpoint_url = os.environ.get("MODAL_TRELLIS_URL", "") or os.environ.get("MODAL_TRIPOSR_URL", "")
        if not endpoint_url:
            raise ValueError("MODAL_TRELLIS_URL/MODAL_TRIPOSR_URL não está configurado.")
